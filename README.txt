...
Organization (NEW in v3.8):
   - Tab sections are collapsible for cleaner workspace
   - "Compute from FITS Files" section - process new observations
   - "Load from Power Spectrum Files" section - load saved .pow files
   - Collapse state remembered across application restarts
   - Click section title or arrow button to expand/collapse
...
1. Computing Fringe Patterns:
   ...
   Saving Power Spectra (NEW in v3.8):
   - "Save as .pow" button saves currently displayed power spectrum
   - Respects reference normalization setting (saves normalized if enabled)
   - Automatically suggests filename with "_normalized" suffix when applicable
   - Saved files can be reloaded later without reprocessing FITS files
   - Includes metadata (target name, date, normalization status, statistics)
...
3. Autocorrelation Features (for Binary Star Analysis):
   ...
   - Multiple Binary Pair Detection: Detects ALL symmetric peak pairs
   - Binary Pair Selector: Dropdown to switch between detected pairs
   - Priority Selection: Always prioritizes strongest peaks first
   - Separation Display: Title shows distance and position angle
   - 1D Slice Plot: Shows intensity profile along binary axis
     * Adjustable center peak suppression radius (2-50 pixels)
     * Green dashed lines mark binary separation distance
     * Log scale y-axis for better visibility

4. Display Window (NEW in v3.8):
   Window Features:
   - Adaptive sizing: Window height scales to screen size (85% of display height)
   - Ensures matplotlib toolbar always visible on smaller displays
   - Collapsible control sections for cleaner workspace
   - Display Options section - expanded by default
   - Display Controls section - collapsed by default for less clutter
   - Collapse state remembered for future display windows
   ...
   - Suppress Center Peak: Remove central autocorrelation peak
     * Radius (px): Adjustable suppression radius (2-50 pixels, default 10)
   - Binary Pair: Dropdown to select which pair to display
   - Show Contours: Overlay contour lines on image
   - Show Markers: Toggle visibility of side lobe markers and binary pair indicators
     * When enabled: Shows red X markers for peaks and lime circles for binary pairs
     * When disabled: Displays clean autocorrelation for unobstructed analysis

   Display Controls:
   - Contrast/Brightness: Adjust image display
   - Percentile Clipping: Control min/max display values
   - Reset Values: Return to default contrast/brightness/percentile settings
   - Reset Zoom: Return to default zoom level

5. Navigation:
   - Mouse wheel: Zoom in/out
   - Toolbar: Pan, zoom, save image options
   - Multiple independent windows can be opened simultaneously
...
- Window preferences
- Fringe tab collapse states (NEW in v3.8)
- Display window control group collapse states (NEW in v3.8)
...
- Added collapsible sections to Fringe Pattern tab
  ...
  * Click title or arrow button to expand/collapse for cleaner workspace
- Added "Save as .pow" button in Fringe tab
  * Saves currently displayed power spectrum with metadata
  * Respects reference normalization setting
  * Automatic filename suggestions with "_normalized" suffix when applicable
  * Saved files can be reloaded without reprocessing FITS files
- Collapsible control groups in fringe display window
  * Display Options section (expanded by default)
  * Display Controls section (collapsed by default)
  * Collapse states remembered for future display windows
  * Reduces screen clutter while maintaining full functionality
- Adaptive display window sizing
  * Window height scales to 85% of screen height
  * Ensures matplotlib toolbar always visible on smaller displays
  * Maximum size limits maintained for larger displays

v3.7.0 - Network Optimization
- Added automatic network drive detection for batch processing
- Optional local temp copy feature for improved performance on network drives
- Reduces FITS file reads from 2-3 times to 1 time (via OS caching)
- Automatic cleanup of temporary files after processing
- Configurable "Copy to local temp" checkbox in batch mode options
- Detects UNC paths (\\server\share) and mapped network drives
- Setting persisted across sessions in config.json
- Significantly improves batch processing speed for network-based workflows

v3.6.0 - Power Spectrum File Management
...
- Checkbox located in Display Controls section
- Binary pair information remains in title when markers are hidden

v3.5.0 - Enhanced Autocorrelation Analysis
- Added adjustable center peak suppression radius control (2-50 pixels, default 10)
- Detects ALL symmetric binary pairs, not just the first one
- Binary pair selector dropdown to switch between multiple detected pairs
- Always prioritizes strongest peaks for binary pair selection
- Pairs sorted by combined intensity with distance and angle displayed
- Improved peak detection applies suppression radius to all analysis steps
- Real-time display updates when switching between pairs
